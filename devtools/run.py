...
journal_stdout_name = "vm-stdout"
journal_stderr_name = "vm-stderr"
...
class MicroVM:
    ...
    async def start_firecracker(self, config_path: Path) -> asyncio.subprocess.Process:
        ...
        if self.enable_log:
            self.journal_stdout = journal.stream(journal_stdout_name)
            self.journal_stderr = journal.stream(journal_stderr_name)
        else:
