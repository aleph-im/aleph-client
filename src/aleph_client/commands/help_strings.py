...
CHANNEL = "Aleph Cloud network channel where the message is or will be broadcasted"
...
ROOTFS_SIZE = (
    "Main VM partition size in MiB to allocate. Set to 0 to use the value from COMPUTE_UNITS. Overrides disk only."
)
VCPUS = "Number of virtual CPUs to allocate. Set to 0 to use the value from COMPUTE_UNITS. Overrides vCPUs only."
MEMORY = "Maximum memory (RAM) in MiB to allocate. Set to 0 to use the value from COMPUTE_UNITS. Overrides memory only."
...
ADDRESS_PAYER = "Address of the payer. In order to delegate the payment, your account must be authorized beforehand to publish on the behalf of this address. See the docs for more info: https://docs.aleph.cloud/devhub/building-applications/messaging/permissions.html#message-permissions"
